...
- **Watchdog is a "don't-start-next-pass" gate, not a true interrupt.** The
  ...
  latency*.
- **Contradiction gate trusts only a user-supplied `hisgrid`.** A harvested grid
  drives a8d recovery but never suppresses a target-sender row, so a ~1/1024
  hash-collision-poisoned grid can't silently drop the real target. (Design note,
  not a gap; guarded by `harvested_grid_never_suppresses_the_real_target`.)
