...
- **P2.0 建后端边界**：在 `ft8v2/osd174_91.rs` 旁抽出 OSD 内部的 GF(2) 原语
  （高斯消元 row-reduce、矩阵×向量 mod 2、syndrome/重编码、错误图样枚举）为一组
  可替换函数；默认实现 = 现有标量逻辑，零行为变化（先验证基线仍绿）。
...
P2 ▶ bit-exact SIMD OSD（主攻 40%，保对齐）
