...
fn wsjtx_ft8_params_constants_match_rust_params() {
    ...
    let rust = read_to_string(&project_root().join("src/ft8/decode/ft8_params.rs"));
...
fn wsjtx_ft8_downsample_shape_matches_shared_rust_helper() {
    ...
    let params = read_to_string(&project_root().join("src/ft8/decode/ft8_params.rs"));
    let rust = read_to_string(&project_root().join("src/ft8/decode/ft8_downsample.rs"));
    let ap = read_to_string(&project_root().join("src/ft8/ap_decode.rs"));
...
fn wsjtx_sync8d_shape_matches_shared_rust_costas_sync() {
    ...
    let rust = read_to_string(&project_root().join("src/ft8/decode/costas_sync.rs"));
    let ft8b = read_to_string(&project_root().join("src/ft8/decode/ft8b.rs"));
    let ap = read_to_string(&project_root().join("src/ft8/ap_decode.rs"));
    ...
    for expected in [
        ...
        assert!(
            rust.contains(expected),
            "Rust costas_sync missing {expected}"
        );
    }
