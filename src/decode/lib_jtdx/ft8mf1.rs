...
fn tonesd_messages(c1: &str, c2: &str, grid: &str) -> Option<Vec<Ft8mf1Message>> {
    const RPT: [&str; 75] = [
    ...
    for rpt in RPT {
        out.push(build_message(&format!("{c1} {c2} {rpt}"))?);
    }
    out.push(build_message(&format!("{c1} {c2} {grid}"))?);
    Some(out)
}
...
fn build_message(msg: &str) -> Option<Ft8mf1Message> {
