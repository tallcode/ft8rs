...
extern "C" {
    ...
    fn fftw_plan_dft_1d(n: i32, input: *mut f64, output: *mut f64, sign: i32, flags: u32) -> PlanHandle;
...
impl PlanAndBuffers {
    fn r2c(n: usize) -> Self {
        ...
        let plan = unsafe {
            fftw_plan_dft_r2c_1d(n as i32, buf_in.as_mut_ptr(), buf_out.as_mut_ptr(), FFTW_ESTIMATE)
        };
        if plan.is_null() { panic!("FFTW r2c plan failed for n={n}"); }
        Self { plan, buf_in, buf_out }
    }
    ...
    fn c2r(n: usize) -> Self {
        ...
        let plan = unsafe {
            fftw_plan_dft_c2r_1d(n as i32, buf_in.as_mut_ptr(), buf_out.as_mut_ptr(), FFTW_ESTIMATE)
        };
        if plan.is_null() { panic!("FFTW c2r plan failed for n={n}"); }
        Self { plan, buf_in, buf_out }
    }
    ...
    fn c2c(n: usize, forward: bool) -> Self {
        ...
        let plan = unsafe {
            fftw_plan_dft_1d(n as i32, buf_in.as_mut_ptr(), buf_out.as_mut_ptr(), sign, FFTW_ESTIMATE)
        };
        if plan.is_null() { panic!("FFTW c2c plan failed for n={n}"); }
        Self { plan, buf_in, buf_out }
    }
}
...
impl PlanCache {
    fn new() -> Self {
        Self {
            r2c: HashMap::new(), c2r: HashMap::new(),
            c2c_fwd: HashMap::new(), c2c_bwd: HashMap::new(),
        }
    }
    ...
    fn get_c2c(&mut self, n: usize, forward: bool) -> &mut PlanAndBuffers {
        if forward {
            self.c2c_fwd.entry(n).or_insert_with(|| PlanAndBuffers::c2c(n, true))
        } else {
            self.c2c_bwd.entry(n).or_insert_with(|| PlanAndBuffers::c2c(n, false))
        }
    }
}
...
mod tests {
    ...
    fn c2r_roundtrip() {
        ...
        let s = 1.0 / n as f64;
        for i in 0..n { re[i] *= s; }
        for i in 0..n { assert!((re[i] - orig[i]).abs() < 1e-9, "{i}"); }
    }
    ...
    fn fftw_3840_sanity() {
        ...
        let n = 3840;
        let mut re: Vec<f64> = (0..n).map(|i| (i as f64 * 2.0 * std::f64::consts::PI * 50.0 / n as f64).cos()).collect();
        let mut im = vec![0.0; n];
        fft_r2c(&mut re, &mut im);
        // Check peak is at bin 50 (50 Hz signal)
        let max_bin: usize = (0..n/2+1).max_by(|&a, &b| 
            (re[a]*re[a]+im[a]*im[a]).partial_cmp(&(re[b]*re[b]+im[b]*im[b])).unwrap()
        ).unwrap();
        assert_eq!(max_bin, 50, "Peak should be at bin 50, got {}", max_bin);
        assert!(re[50].abs() > n as f64 * 0.3, "Peak magnitude too low: {}", re[50]);
    }
